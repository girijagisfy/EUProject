...
namespace CF
{
    public partial class PrevalentDiseasesInfo : System.Web.UI.Page
    {
        ...
        {
            string query = "select Disid,NameofHamlet,Hamletid,DiseasesName, description from tblDiseasesinvillage a left outer join tblHamletData b on a.HdId=b.HDid left outer join tblHamletInfo c on b.Hid=c.Hid where a.HdId=" + Session["HDid"];
        ...
        protected void gvDiseases_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("btnEdit"))
            {
                ...
                string val = (string)this.gvDiseases.DataKeys[rowIndex]["Disid"].ToString();
                Session["Disid"] = val;
                Response.Redirect("EditDiseases.aspx");
            }
        }
        ...
        protected void gvDiseases_Sorting(object sender, GridViewSortEventArgs e)
        {
            ...
            if (dtrslt.Rows.Count > 0)
            {
                ...
                gvDiseases.DataSource = dtrslt;
                gvDiseases.DataBind();
            }

            for (int i = 0; i < gvDiseases.Columns.Count; i++)
            {
                string lbText = gvDiseases.Columns[i].SortExpression;

                if (lbText == e.SortExpression)
                {
                    TableCell tableCell = gvDiseases.HeaderRow.Cells[i];
                    Image img = new Image();
        ...
        protected void gvDiseases_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            string val = (string)this.gvDiseases.DataKeys[rowIndex]["Disid"].ToString();
            string deleteQ = "delete from tblDiseasesinvillage where Disid=" + val;
            if (db.UpdateQuery(deleteQ, "", "", "") > 0)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "ShowAlert('You have deleted Diesease successfully.','success')", true);
        ...
        {
            string query = "select NameofHamlet,Hamletid,DiseasesName, description from tblDiseasesinvillage a left outer join tblHamletData b on a.HdId=b.HDid left outer join tblHamletInfo c on b.Hid=c.Hid where a.HdId=" + Session["HDid"];
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                using (XLWorkbook wb = new XLWorkbook())
                {
                    ...
                    using (MemoryStream stream = new MemoryStream())
                    {
                        wb.SaveAs(stream);
                        stream.Position = 0;
                        string myName = Server.UrlEncode("PrevalentDiseases_" + DateTime.Now.ToShortDateString() + ".xlsx");
        ...
        private void ExportDataSetToExcel(DataSet ds)
        {
            ...
            try
            {
                string filename = "PrevalentDiseases" + DateTime.Now.ToString("dd-MM-yyyy") + "_" + DateTime.Now.ToString("hh-mm-ss") + ".xlsx";
